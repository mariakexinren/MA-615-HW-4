...
# select unique rows from the data group of who3, country, iso2 and iso3
  group_by(country) %>%
  filter(n() > 1)
# When we group together the three columns country, iso2 and iso3, and try to findunique rows from the data, we found that none of the values in the columns have different values from each other, therefore these three columns are redundant.
```
